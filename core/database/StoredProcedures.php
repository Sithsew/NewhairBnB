...
//    }
    ...
    function BSP_SalonGetTopSalons($limit)
    {
        $statement = $this->pdo->prepare("call BSP_SalonGetTopSalons($limit)");
    ...
    {
        $statement = $this->pdo->prepare("call BSP_StylistGetTopStylists($limit)");
    ...
    {
//        prepare("select email from trn_user WHERE email='$email' ")
        $statement = $this->pdo->prepare("call BSP_UserCheckEmail($email)");
...
//        $statement = $this->pdo->prepare("select id from trn_user WHERE email='$email' AND temp_password= '$tempPassword'");
        $statement = $this->pdo->prepare("call BSP_UserCheckPassHash($email, $tempPassword)");
...
//        $statement = $this->pdo->prepare("select id , firstname, lastname, user_role from trn_user WHERE email='$email' AND password= '$password' AND active=1");
        $statement = $this->pdo->prepare("call BSP_UserCheckUser($email, $password)");
...
//        $statement = $this->pdo->prepare("select id from trn_user WHERE email='$email' AND password= '$password' ");
        $statement = $this->pdo->prepare("call BSP_UserCheckUserToActivate($email, $password)");
    ...
    {
//        $statement = $this->pdo->prepare("select id, firstname, lastname, user_role from trn_user WHERE email='$email' AND password= '$password' AND active=1");
        $statement = $this->pdo->prepare("call BSP_UserGetDetails($email, $password)");
...
//        $sql = "UPDATE trn_user SET active = 1 WHERE  email = '$email'";
        ...
        $sql = "call CSP_UserActivate($email)";
